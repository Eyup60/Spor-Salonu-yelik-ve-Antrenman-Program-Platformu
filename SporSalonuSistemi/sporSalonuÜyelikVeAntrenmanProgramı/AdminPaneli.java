...
public class AdminPaneli extends JFrame {
    ...
    private Admin admin; // Giriş yapmış olan yönetici nesnesi
    private JTable table; // Kullanıcı verilerinin listelendiği tablo
    private DefaultTableModel tableModel; // Tablo veri modeli
    ...
    public AdminPaneli(Admin admin) {
        ...
        String[] cols = {"ID", "Email", "Rol", "Sorumlu Antrenör"};
        tableModel = new DefaultTableModel(cols, 0) {
            @Override
            public boolean isCellEditable(int row, int column) {
                // TABLO HÜCRELERİNİN DOĞRUDAN DÜZENLENMESİNİ ENGELLER (READ-ONLY)
                return false; 
            }
        };
    ...
    private void verileriYukle() {
        ...
        java.util.List<Kullanici> tumKullanicilar = Admin.getKullanicilar();

        // ÜYE ID -> ANTRENÖR ADI eşleşmesini Antrenor nesnelerinden oluşturur
        java.util.Map<String, String> uyeAntrenorMap = new java.util.HashMap<>();
        for (Kullanici k : tumKullanicilar) {
            if (k instanceof Antrenor antrenor) {
                String antrenorAdi = antrenor.getIsim() + " " + antrenor.getSoyisim();
                for (Uye uye : antrenor.listele()) {
                    uyeAntrenorMap.put(uye.getId(), antrenorAdi);
                }
            }
        }

        // TÜM KULLANICILARI TABLOYA EKLEME DÖNGÜSÜ
        for (Kullanici k : tumKullanicilar) {
            String antrenorBilgisi = "---";
            // Kullanıcı bir ÜYE ise harita üzerinden sorumlu antrenörü bulur
            if (k.getRole() == Role.UYE) {
                antrenorBilgisi = uyeAntrenorMap.getOrDefault(k.getId(), "Atanmadı");
            }
            tableModel.addRow(new Object[]{k.getId(), k.getEmail(), k.getRole().name(), antrenorBilgisi});
        }
    }
    ...
    private void ekleKullanici() {
        ...
        // SADECE YETKİLİ ROLLERİN (ADMIN/ANTRENOR) SEÇİLMESİNİ SAĞLAR
        dialog.setRestrictedRoles(new Role[]{Role.ADMIN, Role.ANTRENOR});
        dialog.setVisible(true);
        
        Kullanici yeni = dialog.getKullanici();
        if (yeni != null) {
            admin.ekle(yeni); // Nesne üzerinden sisteme ekle
    ...
    private void guncelleKullanici() {
        ...
        if(sonuc != null) {
            admin.guncelle(sonuc); // Veriyi güncelle
            setTitle("Yönetici Paneli - " + admin.getEmail()); // Başlığı güncelle
            verileriYukle(); // Tabloyu tazele
    ...
    private void silKullanici() {
        ...
        if (reply == JOptionPane.YES_OPTION) {
            ...
            try {
                admin.sil(id); // Admin yetkisiyle silme işlemi
                verileriYukle();
            } catch (IllegalStateException ex) {
                // KRİTİK HATALARI (ÖRN: SON ADMİNİN SİLİNMESİ) KULLANICIYA BİLDİRİR
    ...
    // FİYAT GÜNCELLEME EKRANINI AÇAN METOT
    private void fiyatlariGuncelle() {
        FiyatGuncellemeDialog dialog = new FiyatGuncellemeDialog(this);
        dialog.setVisible(true);
        verileriYukle(); // Fiyat değişimleri sonrası tabloyu tazele
    }
        ...
        for (int i = 0; i < buttons.length; i++) {
            ...
            currentButton.addKeyListener(new java.awt.event.KeyAdapter() {
                public void keyPressed(java.awt.event.KeyEvent evt) {
                    ...
                    // SAĞ VEYA AŞAĞI OK: SONRAKİ BUTONA ODAKLAN
                    ...
                    // SOL VEYA YUKARI OK: ÖNCEKİ BUTONA ODAKLAN
                    ...
                    // ENTER TUŞU: ODAKLI BUTONU TETİKLE
